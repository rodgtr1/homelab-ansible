...
# Standard Host Setup Script
# This script prepares a Raspberry Pi for general services (Traefik, Portainer, etc.)
# Run this AFTER Pi-hole is deployed and accessible

# Configuration variables
nameserver="192.168.1.92"  # Your Pi-hole DNS server IP
...
# Fallback DNS in case Pi-hole is temporarily unreachable
    ...
    ethernets:
      eth0:
        ...
        - to: default
          ...
          metric: 100
        nameservers:
          addresses: [${nameserver}, ${fallback_nameserver}]
    wifis:
        renderer: networkd
        wlan0:
          dhcp4: false
...
# Configure resolv.conf to use Pi-hole with fallback
...
# Create new resolv.conf with Pi-hole primary, fallback secondary
...
echo ""
echo "=========================================="
echo "Standard host pre-setup complete!"
echo "=========================================="
echo "eth0 IP: $eth0_ip (PRIMARY - metric 100)"
echo "wlan0 IP: $wlan0_ip (BACKUP - metric 200)"
echo "DNS: $nameserver (Pi-hole) with fallback to $fallback_nameserver"
echo ""
echo "⚠️  IMPORTANT NEXT STEPS:"
echo "1. Update inventory/homelab.yml with: ansible_host: $eth0_ip"
echo "2. Ensure SSH access works: ssh pi@$eth0_ip"
echo "3. Test DNS resolution: nslookup google.com"
echo "4. Deploy services: ansible-playbook playbooks/site.yml --limit proxy --ask-vault-pass"
echo ""
